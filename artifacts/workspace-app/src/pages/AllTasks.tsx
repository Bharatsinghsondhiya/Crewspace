...
export function useGetAllTasks() {
  return useQuery({
    queryKey: ["/api/dashboard/all-tasks"],
    queryFn: () => customFetch<TaskResponseItem[]>("/api/dashboard/all-tasks"),
  });
}
    ...
    <div className="p-5 md:p-7 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/45 mt-2 text-sm">View all tasks across the workspaces you are a member of.</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-white/8 p-12 text-center" style={{ background: "rgba(20, 12, 45, 0.7)" }}>
          ...
          <p className="text-white/50 text-sm">There are no tasks in your workspaces yet.</p>
        </div>
      ) : (
